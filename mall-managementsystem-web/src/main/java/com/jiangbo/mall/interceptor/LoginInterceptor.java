...
/**
 * @author Jiangbo Cheng on 2017/11/12.
 */
...
public class LoginInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if(uri.indexOf(IMAGE_CATEGORY) == 0 || uri.indexOf(CSS_CATEGORY) == 0 || uri.indexOf(JS_CATEGORY) == 0 ){
            return true;
        }else if(uri.contains("/toLogin") || uri.contains("/login")){
            return true;
            // 其他情况判断session中是否有key，有的话继续用户的操作
        }else if(SessionUtil.getSessionId(request) != null && redisClientBiz.get(SessionUtil.getSessionId(request), UserVO.class) != null){
            return true;
        }
