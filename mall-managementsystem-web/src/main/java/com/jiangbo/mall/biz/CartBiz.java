...
public class CartBiz {
     ...
     * @return
     */
    public PageInfo<CartVO> getCartsByCondition(CartVO cartVO, int pageIndex, int pageSize){
        try{

            CartBO cartBO = new CartBO();
            BeanUtils.copyProperties(cartVO, cartBO);
            PageInfo<CartBO> cartBOPageInfo = cartService.queryListByCondition(cartBO, pageIndex, pageSize);
            return this.convert(cartBOPageInfo);
        }catch(Exception e){
            throw new MallException(e.getMessage());
        }
    }
    ...
    private PageInfo<CartVO> convert(PageInfo<CartBO> cartBOPageInfo){
        PageInfo<CartVO> pageInfo = new PageInfo<>();
        List<CartVO> activityVOList;
        if(CollectionUtils.isNotEmpty(cartBOPageInfo.getList())){
            activityVOList = new ArrayList<>(cartBOPageInfo.getList().size());
            for(CartBO cartBO : cartBOPageInfo.getList()){
                CartVO cartVO = new CartVO();
                BeanUtils.copyProperties(cartBO, cartVO);
                activityVOList.add(cartVO);
            }
            BeanUtils.copyProperties(cartBOPageInfo, pageInfo);
